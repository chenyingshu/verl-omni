...
For the full installation and quickstart guide, see {doc}`flowgrpo_quickstart`. For algorithm details and other reward training (e.g. JPEG incompressibility), see [Algorithm - Flow-GRPO](../algo/flowgrpo.md).

## Installation

Follow the [installation guide](../start/install.md) to set up the base environment, then install the FlowGRPO-specific dependency:
